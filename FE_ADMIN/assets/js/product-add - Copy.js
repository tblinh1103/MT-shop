...
let uploading = true;
// Ngăn reload / unload khi đang upload
window.addEventListener("beforeunload", (e) => {
  if (uploading) {
    e.preventDefault();
    e.returnValue = ""; // trình duyệt hiện thông báo
  }
});
...
async function saveProduct() {
  ...
  window.addEventListener("beforeunload", () => {
    console.trace("Trang đang bị reload bởi:");
  });
  ...
  const uploadPromises = selectedFiles.map(async (file, i) => {
    ...
    try {
      const res = await fetch("http://localhost:8080/tech-store/api/upload/image", {
        method: "POST",
        headers: {
          Authorization: "Bearer " + token,
        },
  ...
  // imageObjects.push({
  //   imageUrl: "http://res.cloudinary.com/dpzqeta51/image/upload/a10e98c8-ade1-4454-a27b-55c362203f19_1.jpg",
  //   isMain: i === 0,
  // });
  // }
